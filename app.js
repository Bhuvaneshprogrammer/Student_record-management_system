...
const regNo = document.getElementById("regNo"); // ✅ NEW
...
async function save(){
    ...
    if(!student.name || !student.email || !student.age || !student.regNo){
    ...
    if(!emailReg.test(student.email)){
        return toastMsg("Invalid email",1);
    }

    if(!regNoPattern.test(student.regNo)){
        return toastMsg("Invalid Register Number",1);
    }
...
// EXPORT
function exportCSV(){
    let csv="Name,Age,Dept,Email,RegNo\n";

    document.querySelectorAll("#tb tr").forEach(r=>{
        let c=r.children;
        csv+=`${c[0].innerText},${c[1].innerText},${c[2].innerText},${c[3].innerText},${c[4].innerText}\n`;
    });
